...
## 
clubhouse_token ?= $$CLUBHOUSETOKEN
from_mail_address ?= $$EMAILADDRESS
to_mail_address ?= $$EMAILADDRESS
...
build: ## Build the executable for Lambda
	echo
	GOOS=linux GOARCH=amd64 go build -o bin/getstories ./getstories
	echo
...
deploy: clean build ## Deploy the app to AWS Lambda
	...
	aws cloudformation deploy \
		...
		User=$(github_user) \
		Team=$(team) \
		ClubhouseToken=$(clubhouse_token) \
		FromMail=$(from_mail_address) \
		ToMail=$(to_mail_address) \
